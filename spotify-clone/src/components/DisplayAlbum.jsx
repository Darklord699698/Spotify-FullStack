...
const DisplayAlbum = ({album}) => {
  ...
  return albumData ? (
    ...
    <hr />
    {
        songsData.filter((item)=>item.album===album.name).map((items,index)=>(
            <div onClick={()=>playWithId(items._id)} key={index} className='grid items-center grid-cols-3 gap-2 p-2 sm:grid-cols-4 text-[#a7a7a7] hover:bg-[#ffffff2b] cursor-pointer'>
